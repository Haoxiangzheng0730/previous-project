...
class OptimalPathStabilizer:
    ...
    def create_e_vars(self):
        ...
        for wi in range(len(self.weights)):
            e_w = []
            for t in range(self.original_path_length):
    ...
    def create_b_vars(self):
        self.b_vars = []
        for t in range(self.original_path_length):
            self.b_vars.append(LpVariable("%s_%06d" % (self.b_name_prefix, t), lowBound=-self.maximal_windows_shift,
                                          upBound=self.maximal_windows_shift))

    def create_minimization_objective(self):
        objective = []
        for weight_index, weight in enumerate(self.weights):
            objective += [weight * self.e_vars[weight_index][t] for t in range(self.original_path_length)]
        return objective
...
class OptimalPathStabilizerXYA:
    def __init__(self, x, y, a, limits):
        ...
        self.limits = limits
        self.labels = ["x", "y", "a"]
        self.stabilizers = [OptimalPathStabilizer(self.original_transformations[d], limits[d]) for d in range(3)]
